...
check_scripts() {
  ...
  python -m py_compile "scripts/agentic/init_support.py"
...
run_pipeline() {
  ...
  python scripts/agentic/validate-setup-profile.py || return 1
...
case "$COMMAND" in
  ...
  validate-setups)
    ...
    ;;
  validate-setup-profile)
    python scripts/agentic/validate-setup-profile.py
    ;;
  validate-references)
