...
def validate_agent_file(path: Path) -> list[str]:
    ...
    if produces is not None:
        if not isinstance(produces, list):
            errors.append(f"{path}: produces must be a list when present")
        else:
            for artifact_type in produces:
                if not isinstance(artifact_type, str) or not artifact_type.strip():
                    errors.append(f"{path}: produces entries must be non-empty strings")
                    continue

                if not artifact_exists(artifact_type):
                    errors.append(
                        f"{path}: produces missing artifact contract: "
