...
The temporary shell launcher only routes `lock` and `validate-lockfile` to `cli.lockfile_generation` and `cli.lockfile_validation`; it does not own lockfile semantics.
