...
def test_workflow_terminal_state_const_is_enforced(
    ...
    for state_value in states:
        ...
        raise AssertionError(
            "Expected one terminal workflow state."
        )
...
def test_invalid_json_document_is_reported_individually(
    ...
    diagnostic = next(
        ...
        if item.code == DOCUMENT_INPUT_DIAGNOSTIC
    )
    assert diagnostic.source_path == (
        document_path.relative_to(tmp_path).as_posix()
    )
...
def test_invalid_schema_definition_is_reported(
    ...
    diagnostic = next(
        item
        for item in result.diagnostics
        if item.code == SCHEMA_DEFINITION_DIAGNOSTIC
    )
    assert diagnostic.source_path == (
        ".agentic/schemas/registry/agent.schema.json"
    )
    assert "invalid Draft 2020-12 schema" in (
        diagnostic.message
    )
...
def _workflow_path(root: Path) -> Path:
    for candidate in sorted(
        (root / "registry" / "workflows").glob(
            "*.workflow.json"
        )
    ):
        if _read_json(candidate).get("name") == (
            "orchestrated-delivery"
        ):
            return candidate

    raise AssertionError(
        "Expected orchestrated-delivery workflow."
    )
...
def _matching_diagnostic(
    ...
    return next(
        ...
        if diagnostic.source_path == relative_path
